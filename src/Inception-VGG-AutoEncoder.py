...
from keras.layers import RepeatVector, Reshape, Concatenate, Conv2D, UpSampling2D, Conv2D, Input, GlobalAveragePooling2D, Dense, Dropout, BatchNormalization
...
inception = InceptionV3(include_top=True, weights='imagenet', input_shape=(224, 224, 3), input_tensor=input_concat)
...
repeat = RepeatVector(7 * 7)(inception.get_layer('predictions').output)
reshape = Reshape([7, 7, 1000])(repeat)
fusion = Concatenate(axis=3)([vgg16.get_layer('block5_pool').output, reshape])
...
model.fit_generator(train_gen(batch_size),
                    ...
                    callbacks=[model_checkpoint, csv_logger, reduce_lr_on_plateau],
                    shuffle=True,
